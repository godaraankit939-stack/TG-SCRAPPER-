    ...
    while True:
        ...
        try:
            ...
            if msg.media:
                ...
                print(f"\n{YELLOW}Uploading to your ID...{RESET}")
                await app.send_document(chat_id=target_user, document=file_path, caption=msg.caption, progress=progress)
                
                os.remove(file_path)
                print(f"\n{GREEN}[✔] Done! Data saved/sent.{RESET}")
            else:
                await app.send_message(target_user, msg.text)
